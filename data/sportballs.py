...
def _sports_balls_generator(train=True, canvas_size=64, object_size=34, n_objects=1, random_cls=False):
    ...
    while True:
        ...
        colormap_type = 0  # Default to original colormap
        if random_cls:  # for random colormap
            if torch.randint(0, 3, (1,), generator=rnd_gen).item() == 0:
                background = ImageOps.invert(background.convert('RGB'))  # inverted colormap
                colormap_type = 1
            elif torch.randint(0, 3, (1,), generator=rnd_gen).item() == 1:
                background = ImageOps.grayscale(background)  # grayscale colormap
                colormap_type = 2

        # return the class(es) as label
        yield background, class_idx.tolist()[0], colormap_type, rnd_state
...
def store_sports_balls(length, train=True, path='/home/magda/Data/SportBalls', **kwargs):
    ...
    with open(labels_path, 'w', newline='') as lab_file: 
        ...
        for i in range(length):
            img, label, colormap_type, r_state = next(sb)
            if i == 0:
                rnd_state = r_state
            img_name = 'img_' + str(i).zfill(str_len)
            lab_writer.writerow([img_name, label, colormap_type])
